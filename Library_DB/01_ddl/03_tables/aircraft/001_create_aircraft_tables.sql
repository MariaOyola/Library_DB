...
CREATE TABLE aircraft.aircraft (
    ...
    CONSTRAINT ck_aircraft_service_dates 
        CHECK (retired_on IS NULL OR in_service_on IS NULL OR retired_on >= in_service_on)
);
...
CREATE TABLE aircraft.aircraft_seat (
    ...
    CONSTRAINT uq_aircraft_seat_position 
...
CREATE TABLE aircraft.maintenance_event (
    ...
    CONSTRAINT ck_maintenance_event_status 
        CHECK (status_code IN ('PLANNED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED')),

    CONSTRAINT ck_maintenance_event_dates 
