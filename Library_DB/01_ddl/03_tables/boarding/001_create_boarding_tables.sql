...
CREATE TABLE boarding.boarding_group (
...
CREATE TABLE boarding.check_in_status (
...
CREATE TABLE boarding.check_in (
    ...
    check_in_status_id uuid NOT NULL REFERENCES boarding.check_in_status(check_in_status_id),
    boarding_group_id uuid REFERENCES boarding.boarding_group(boarding_group_id),
...
CREATE TABLE boarding.boarding_pass (
    ...
    check_in_id uuid NOT NULL REFERENCES boarding.check_in(check_in_id),
...
CREATE TABLE boarding.boarding_validation (
    boarding_validation_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    boarding_pass_id uuid NOT NULL REFERENCES boarding.boarding_pass(boarding_pass_id),
    ...
    CONSTRAINT ck_boarding_validation_result 
    CHECK (validation_result IN ('APPROVED', 'REJECTED', 'MANUAL_REVIEW'))
);